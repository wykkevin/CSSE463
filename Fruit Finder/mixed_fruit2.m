...
clear;
clc;
img=imread('fruit\mixed_fruit3.tiff');
hsv=rgb2hsv(img);
imtool(hsv);
...
imtool(maskAAA);
imwrite(maskAAA,'pic2_raw_apple_mask.png');
%%imtool(maskAAA);
%find banana
...
imtool(maskBBB);
imwrite(maskBBB,'pic2_raw_banana_mask.png');
...
maskOOO(orangebb)=0;
imtool(maskOOO);
imwrite(maskOOO,'pic2_raw_orange_mask.png');
...
imwrite(maskAA,'pic2_dilate_mask_apple.png');
%imtool(maskAA);
%imtool(maskA);
%banana
str3=strel('square',4);
...
face=repmat(maskBBB,[1,1,3]).* double(hsv);
%imtool(double(face));
